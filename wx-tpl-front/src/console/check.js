export default async function (response) {
    ...
    switch (mock_matches.length) {
        ...
        case 1:
            {
                const mock_template = mock_matches[0].mock_template;

                if (['object', 'string'].includes(typeof mock_template)) {
                    let result = Mock.valid(mock_template, data);

                    if (result.length) {
                        console.error('check 不通过');
                        console.error('check 规则', mock_template);
                    ...
                    console.error(`当前规则`, mock_template);
                }
            }
            break;
        default:
